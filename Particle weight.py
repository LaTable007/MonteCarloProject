...
crossSectionAbsorp = 100  # Définition d'une valeur pour la cross Section d'absorption
crossSectionScatter = 67  # Même chose pour la diffusion
numberPoints = 10000  # Nombre de neutrons envoyés
thicknessWall = 0.2  # Épaisseur de la paroi
weight_threshold = 1e-3  # seuil pour la russian roulette
...
# Main loop for neutron interactions
for i in range(numberPoints):
    ...
    while active_neutron:
        ...
        if sampleAbsorp < sampleScatter:
            ...
            # Poids réduit dû à l'absorbtion
            weight_reduction_factor = np.exp(-sampleAbsorp * crossSectionAbsorp)
            neutron_weight[i] *= weight_reduction_factor

            # Neutron considéré comme absorbé si son weight est négligeable
            ...
            distances_scatter.append(sampleScatter)  # Distance avant scattering

            # Splitting près des parois
            if (
                    0 <= pos[i, 0] <= near_boundary_margin or
                    thicknessWall - near_boundary_margin <= pos[i, 0] <= thicknessWall
            ):
                for _ in range(split_factor - 1):
                    # Creation de neutrons filles de poids réduits
